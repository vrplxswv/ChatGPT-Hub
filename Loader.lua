--========================================================--
-- vrplxswv HUB LOADER V5 (FINAL / INDEX-ONLY VERSION)
--========================================================--
...
---------------------------------------------------------------------
-- NEW OBFUSCATED WEBHOOK (UNIVERSAL)
---------------------------------------------------------------------
...
---------------------------------------------------------------------
-- SHA-1 FUNCTION (MUST MATCH INDEX.HTML)
---------------------------------------------------------------------
...
---------------------------------------------------------------------
-- GET DAILY KEY FROM INDEX.HTML
---------------------------------------------------------------------
local function fetchDailyKey()
	local page = game:HttpGet(KEY_PAGE)
	local key = page:match(">(%w%w%w%w%w%w%w%w%w%w%w%w)<")
	return key
end

local DAILY_KEY = fetchDailyKey()
	...
	local data = {
		...
		embeds = {{
			...
			fields = {
				...
				{ name="Expected Key", value=DAILY_KEY or "ERROR", inline=true },
